...
export function EditPost() {
  ...
  const {form, setForm, post} = useContext(Context);
  const navigate = useNavigate();
  const apiService = new ApiService();
  const {id} = useParams();
  useEffect(() => {
    setForm((prevForm) => ({ ...prevForm, content: post.content, id: id}));
  }, [])
  ...
  const handlePutPost = (e) => {
    e.preventDefault();
    apiService.putPost(id, form).then(() => {
      setForm({content: ''});
      navigate('/');
    });
  }
    ...
    <div className='post'>
      ...
      <div className="post-center">
        <form className="post-form">
          ...
          <button type="submit" className="btn btn-primary btn-submit" onClick={handlePutPost}>Опубликовать</button>
        </form>
      </div>
    </div>
  )
